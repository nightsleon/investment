...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","Segoe UI",sans-serif;background:#fff;color:var(--text)}}
    .wrap{{max-width:1380px;margin:0 auto;padding:16px 24px 20px}}
    .panel{{background:#fff;border:1px solid var(--line);border-radius:24px;box-shadow:none;overflow:hidden}}
    .head{{padding:16px 24px;border-bottom:1px solid var(--line);display:flex;gap:12px;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;background:linear-gradient(180deg,rgba(255,255,255,1) 0%,rgba(250,252,255,1) 100%)}}
    .title h2{{font-size:20px;margin:0;letter-spacing:-.025em}} .title p{{margin:0;color:var(--muted);font-size:12px}}
    ...
    .head-meta{{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}}
    .head-pill{{display:inline-flex;align-items:center;height:28px;padding:0 10px;border:1px solid var(--line);border-radius:999px;background:#fff;color:var(--muted);font-size:11px;font-weight:600}}
    .head-pill b{{color:var(--text);margin-left:4px}}
    .section{{padding:12px 24px;border-bottom:1px solid var(--line)}}
    ...
    .quick-links-combined{{display:flex;gap:10px;flex-wrap:wrap;align-items:center}}
    .quick-links-combined .group-sep{{width:1px;height:24px;background:var(--line);margin:0 2px}}
    ...
    iframe{{display:block;width:100%;height:calc(100vh - 170px);border:0;background:#fff}}
    @media (max-width:900px){{.wrap{{padding:12px}} .head{{padding:14px 16px;align-items:flex-start}} .title{{width:100%}} .controls{{width:100%}} select,button,a.btn{{flex:1;min-width:0}} .section{{padding:12px 16px}} .mobile-note{{display:block;padding:12px 16px}} .mobile-actions{{display:flex;padding:14px 16px}} iframe{{display:none}}}}
    @media (max-width:640px){{.controls label{{width:100%}} select{{width:100%;flex:initial}} button,a.btn{{width:100%}} .mobile-actions a{{min-width:100%}} .quick-links .chip,.archive-nav .chip,.quick-links-combined .chip{{flex:1}} .quick-links-combined .group-sep{{display:none}}}}
  </style>
</head>
<body>
  <div class="wrap">
    <div class="panel">
      <div class="head">
        <div class="title">
          <h2>浏览与切换</h2>
          <div class="head-meta"><span class="head-pill">最新日期<b>{latest}</b></span><span class="head-pill">当前选择<b id="currentDate">{selected}</b></span><span class="head-pill">数据源<b>蛋卷基金</b></span></div>
        </div>
        <div class="controls">
      ...
      <div class="section">
        ...
        <div class="quick-links-combined">
