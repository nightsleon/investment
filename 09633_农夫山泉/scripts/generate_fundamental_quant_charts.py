...
def plot_product(product_path: Optional[Path], out_dir: Path, code: str, company: str):
    ...
    for y, inc_val, yoy in zip(inc[cols["business"]], inc["__increment"], inc[cols["yoy"]]):
        axes[1].annotate(f"{inc_val:+.0f}亿 / {yoy:+.1f}%", xy=(inc_val, y), xytext=(12, 0), textcoords="offset points",
                         va="center", ha="left", fontproperties=FONT, fontsize=9, color=COLORS["green"] if inc_val >= 0 else COLORS["red"])
    max_abs = max(abs(inc["__increment"].min()), abs(inc["__increment"].max()), 1)
    axes[1].set_xlim(min(inc["__increment"].min() * 1.35, -max_abs * 0.12), inc["__increment"].max() * 1.22)
...
def plot_operating_quality(df: pd.DataFrame, out_dir: Path, code: str, company: str, recent_years: int):
    cols = require(df, {
        ...
        "receivable": ["应收账款", "应收款项", "trade_receivables", "receivable"],
