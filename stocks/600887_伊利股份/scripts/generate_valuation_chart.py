#!/usr/bin/env python3
"""Generate Yili ten-year PE valuation chart."""
...
ax.annotate(f"当前 {latest['PE_TTM']:.1f}倍\n近十年约5.1%分位", (latest["TRADE_DATE"], latest["PE_TTM"]), xytext=(-100, 28), textcoords="offset points", fontproperties=FONT, color="#DC2626", arrowprops={"arrowstyle": "->", "color": "#DC2626"})
ax.text(v["TRADE_DATE"].iloc[10], q50 + 0.6, f"中位数 {q50:.1f}倍", fontproperties=FONT, color="#64748B")
ax.set_title("伊利股份：近十年PE(TTM)区间", fontproperties=FONT, fontsize=17, weight="bold", pad=18)
ax.text(0.5, 1.01, "数据源：东方财富估值历史；剔除非正PE；色带表示样本分位区间", transform=ax.transAxes, ha="center", va="bottom", fontproperties=FONT, fontsize=10, color="#64748B")
...
path = OUT / "600887_5.2_近十年PE区间图.png"
fig.savefig(path, dpi=180, bbox_inches="tight", facecolor="white")
print(path)
print(json.dumps({"q10": q10, "q25": q25, "median": q50, "q75": q75, "q90": q90, "latest": float(latest["PE_TTM"])}, ensure_ascii=False))
