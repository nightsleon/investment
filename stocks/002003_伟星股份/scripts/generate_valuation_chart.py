#!/usr/bin/env python3
"""Generate Weixing Share available-history PE valuation chart."""
...
start = v["TRADE_DATE"].min().strftime("%Y-%m-%d")
end = v["TRADE_DATE"].max().strftime("%Y-%m-%d")
...
ax.annotate(f"当前 {latest['PE_TTM']:.1f}倍\n可用历史约{percentile:.1f}%分位", (latest["TRADE_DATE"], latest["PE_TTM"]), xytext=(-135, 32), textcoords="offset points", fontproperties=FONT, color="#DC2626", arrowprops={"arrowstyle": "->", "color": "#DC2626"})
ax.text(v["TRADE_DATE"].iloc[min(10, len(v)-1)], q50 + 0.6, f"中位数 {q50:.1f}倍", fontproperties=FONT, color="#64748B")
ax.set_title("伟星股份：可用历史PE(TTM)区间", fontproperties=FONT, fontsize=17, weight="bold", pad=18)
ax.text(0.5, 1.01, f"数据源：东方财富；{start}至{end}；剔除非正PE（不足十年）", transform=ax.transAxes, ha="center", va="bottom", fontproperties=FONT, fontsize=10, color="#64748B")
...
path = OUT / "002003_5.2_可用历史PE区间图.png"
fig.savefig(path, dpi=180, bbox_inches="tight", facecolor="white")
print(path)
print(json.dumps({"start": start, "end": end, "q10": q10, "q25": q25, "median": q50, "q75": q75, "q90": q90, "latest": float(latest["PE_TTM"]), "percentile": percentile}, ensure_ascii=False))
