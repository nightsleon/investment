#!/usr/bin/env python3
"""Generate Weixing Share quarterly price vs profit charts from Tonghuashun XLS."""
...
CHARTS = BASE / "charts"
DATA = BASE / "data"
XLS = DATA / "002003_main_simple.xls"
CODE = "002003"
COMPANY = "伟星股份"
...
def fetch_price() -> pd.DataFrame:
    raw = yf.download("002003.SZ", start="2008-01-01", auto_adjust=False, progress=False)
...
def match_price(financials: pd.DataFrame, prices: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in financials.dropna(subset=["non_gaap_ttm_yi"]).iterrows():
        ...
        if available.empty:
            continue
        output.append({**row.to_dict(), **available.iloc[-1].to_dict()})
...
def ticks_for(frame: pd.DataFrame) -> list[int]:
...
def plot_dual(frame: pd.DataFrame) -> None:
    x = range(len(frame)); ticks = ticks_for(frame)
...
def plot_single_quarter(frame: pd.DataFrame) -> None:
    ...
    title(ax1, f"{COMPANY}：股价与单季度扣非净利润", "单季度波动包含季节性，TTM图为主要判断依据")
...
def main() -> None:
    ...
    if merged.empty:
        raise RuntimeError("No matched price/profit data")
    merged["quarter_label"] = merged["quarter"]
    merged["profit_ttm_yi"] = merged["non_gaap_ttm_yi"]
    DATA.mkdir(parents=True, exist_ok=True)
    merged.to_csv(DATA / f"{CODE}_季度数据.csv", index=False)
    plot_dual(merged); plot_normalized(merged); plot_single_quarter(merged)
