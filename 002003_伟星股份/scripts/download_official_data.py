#!/usr/bin/env python3
"""Download Weixing Share (002003.SZ) official filings and Eastmoney datasets."""
...
def query(org_id: str, start: str, end: str, category: str = "", searchkey: str = "") -> list[dict]:
    payload = {
        "pageNum": 1,
        "pageSize": 100,
        "column": "szse",
...
def main() -> None:
    ...
    annual_items = query(org_id, "2021-01-01", CUTOFF, "category_ndbg_szsh")
    manifest: list[dict] = []
    for year in range(2020, 2026):
        ...
        manifest.append(download_pdf(matches[0], REPORTS, f"{year}_伟星股份_年度报告.pdf"))

    all_2026: list[dict] = []
    for month in range(1, 8):
        start = f"2026-{month:02d}-01"
        end = CUTOFF if month == 7 else f"2026-{month + 1:02d}-01"
        all_2026.extend(query(org_id, start, end))
    for key in ("年度报告", "审计报告", "减持", "质押", "回购", "激励", "分红", "对外投资", "收购", "问询函", "监管", "处罚"):
        all_2026.extend(query(org_id, "2026-01-01", CUTOFF, searchkey=key))
    ...
    manifest.append(download_pdf(q1[-1], REPORTS, "2026Q1_伟星股份_第一季度报告.pdf"))

    keywords = (
        "权益分派", "利润分配", "回购", "减持", "质押", "激励", "员工持股",
        "董事", "监事", "高级管理人员", "总经理", "投资", "收购", "关联交易",
        "股东大会决议", "监管", "问询", "业绩快报", "担保",
    )
