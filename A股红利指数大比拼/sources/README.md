...
- `etf-history-price/`：ETF行情元数据、前复权价格、持仓阶段收益回撤，以及复权净值对全收益指数的跟踪差复算
...
- 深证红利绩效暂用跟踪该指数的红利ETF工银（159905）复权单位净值代理。原始净值归档于 `etf-history-price/159905_历史净值_东方财富_2010-11-05_2026-08-05.json`，统一比较序列为 `399324_深证红利_159905复权净值代理_2016-06-30_2026-07-31.csv`。
- ETF代理已经扣除基金费用，并受跟踪误差影响，不等于官方税前全收益指数；文章只用它判断大致风险收益位置，不把细小差异写成指数精确差距。
- 沪深300全收益指数（H00300）作为宽基基准，数据来源 `https://www.csindex.com.cn/csindex-home/perf/index-perf?indexCode=H00300&startDate=20160630&endDate=20260731`，归档于 `performance-data/H00300_沪深300全收益_2016-06-30_2026-07-31.csv`。
